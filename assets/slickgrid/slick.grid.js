...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function appendRowHtml(stringArray, row, range) {
      ...
      for (var i = 0, ii = columns.length; i < ii; i++) {
        ...
        if (columnPosRight[Math.min(ii - 1, i + colspan - 1)] > range.leftPx) {
          ...
          appendCellHtml(stringArray, row, i, colspan, metadata);
        }
    ...
    function appendCellHtml(stringArray, row, cell, colspan, metadata) {
      var m = columns[cell];
      var d = getDataItem(row);
    ...
    function viewPortWidth() {
        ...
        // if the container isn't visible we may get a width of zero which will result in cells not getting rendered
        // (haven't investigated why) overriding the 0 with 200 seems to fix the problem.
        if (width === 0) {
            width = 200;
        }
    ...
    function clearTextSelection() {
      self.clearingTextSelection = true;
      ...
      self.clearingTextSelection = false;
    }
